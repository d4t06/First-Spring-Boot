...
public class ProductSpecs {
   ...
   public static Specification<Product> containName(String name) {
      ...
      ConvertEng converter = new ConvertEng();

      return (root, query, criteriaBuilder) -> criteriaBuilder
            .like(criteriaBuilder.lower(root.get("product_name_ascii")), "%" + converter.convert(name) + "%");
   }
   ...
   public static Specification<Product> betweenPrice(Integer fromPrice, Integer toPrice) {
      return (root, query, criteriaBuilder) -> {
         ...
         return criteriaBuilder
               .between(
                     defaultStorageJoin.<Integer>get("storage").get("defaultStorageCombine").get("combine")
                           .get("price"),
                     fromPrice,
                     toPrice);
      };
